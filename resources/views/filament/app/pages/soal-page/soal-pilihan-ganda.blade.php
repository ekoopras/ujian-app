<div class="space-y-3">
    ...
    $isSelected = ($jawabanSiswa[$soalAktif->id] ?? '') === $teksJawaban;
    @endphp

    <label
        wire:key="opt-pg-{{ $soalAktif->id }}-{{ $idx }}"
        ...
        <input
            type="radio"
            name="jawaban_{{ $soalAktif->id }}"
            value="{{ $teksJawaban }}"
            wire:click="simpanJawaban({{ $soalAktif->id }}, '{{ addslashes($teksJawaban) }}')"
            @checked($isSelected)
            class="mt-1 text-indigo-600 focus:ring-indigo-500 border-gray-300 dark:border-gray-600">
